...
public class Graph {
  ...
  /*TODO: Create a copy constructor that will allow you to copy and manipulate
   * the private arrays of a Graph object. The link list are already public...
   * double check to see if it is NECESSARY to have the private arrays copied
   * over as well. (Hanslo Withams) */ 
							...
							for(int i = 0; i < attributeCount; i++){
								if(xrp.getAttributeName(i).equals("k")
									...
									tempWay.setIndoor(v.equals("yes"));
										...
										&& xrp.getAttributeValue(i).equals("wallPath")){			// way.tag.wallPath
									String v = xrp.getAttributeValue(i + 1);
									tempWay.setWallPath(v.equals("yes"));
                }else if(xrp.getAttributeName(i).equals("k")
										&& xrp.getAttributeValue(i).equals("walkPath")){			// way.tag.walkPath
									String v = xrp.getAttributeValue(i + 1);
									tempWay.setWalkPath(v.equals("yes"));
                }else if(xrp.getAttributeName(i).equals("k")
	...
	public Stack<GraphNode> getShortestPath(String from, String to){
		...
		return getShortestPath(gnFrom, gnTo);
	}
	...
	public Stack<GraphNode> getShortestPath(GraphNode from, GraphNode to){
		...
		if(from == null || to == null){
			Log.i("getShortestPath", "from or to is null");
			return null;
		}
		Log.i("GraphNode from", from.getName());
		Log.i("GraphNode to", to.getName());
		int remaining_nodes = array_nodes_by_id.length;
		Log.i("remaining_nodes", Integer.toString(remaining_nodes));
		...
		Log.i("dist:", Integer.toString(getNodePosInIdArray(from)));
		int count = 0;
		while(remaining_nodes>0){
			...
			if(u != null) {
				Log.i("u_i", Integer.toString(u_i));
			}
			else
				Log.i("u: ", "NULL");
			...
			LinkedList<GraphNode> nOuIq = getNeighbours(visited, u);

			if(u.equals(to)){
				Log.i("u.equal(to)", "YAY");
	...
	private LinkedList<GraphNode> getNeighbours(boolean[] visited, GraphNode node){
		int getNeighnorCount = 0;
		LinkedList<GraphNode> ret = new LinkedList<GraphNode>();
		for(GraphEdge edge : node.getLocEdges()){
			getNeighnorCount++;
			// check all edges if they contain node
			GraphNode buf = null;
			...
			// if !outside, only indoor nodes are allowed
				if(buf!=null){
					...
					if(buf == null)
					Log.i("getNeighbours:", "buf is null");
					else
						Log.i("getNeighbours:", "buf is not indoor");
				}
			}
		Log.i("getNeighborCount: ", Integer.toString(getNeighnorCount));
		return ret;
	}
	 ...
	 * @return the distance in meters
	 */
  //TODO: Override this function for the visually disabled. (Hanslo Withams)
