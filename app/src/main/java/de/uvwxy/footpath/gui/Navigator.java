...
public class Navigator extends Activity implements StepTrigger{
	...
	public void trigger(long now_ms, double compDir) {
		...
		//if(pbMap.isAudioFeedBackUpdated())
		//TextToSpeechFunction(pbMap.getAudioFeedBack());
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		//mTextToSpeech = new TextToSpeech(Navigator.this, Navigator.this);
		
		// Get location and destination
		
		nodeFrom 	= 	this.getIntent().getStringExtra("from");
		Log.i("nodeFrom: ",nodeFrom);
		...
		// calculate route
		if(nodeFromId==0){
			navPathStack = this.g.getShortestPath(nodeFrom, nodeTo);
			Log.i("nodeFromId: ", "0");
			logger = new DataLogger(this, System.currentTimeMillis(), nodeFrom, nodeTo);
		} else {
			navPathStack = this.g.getShortestPath(nodeFromId, nodeTo);
			Log.i("nodeFromId: ", Integer.toString(nodeFromId));
		...
		if(navPathStack != null){											// no route found!
			...
			// navPathStack was null
			Log.i("navPathStack", "NULL");
	...
	public void onInit(int Text2SpeechCurrentStatus) {
		...
		if (Text2SpeechCurrentStatus == TextToSpeech.SUCCESS) {
			...
			Log.v("Navigator.java", "onInit_Success");
		}

	}

	public void TextToSpeechFunction(String audioFB)
	{

		String textholder = audioFB;

		mTextToSpeech.speak(textholder, TextToSpeech.QUEUE_FLUSH, null);
	}*/
