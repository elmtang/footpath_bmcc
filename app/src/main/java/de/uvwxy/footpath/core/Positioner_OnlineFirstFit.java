...
public class Positioner_OnlineFirstFit extends Positioner{
	 ...
	 * the user's whereabouts needed to be updated
	 */
...
//				Log.i("FOOTPATH", "Current edge matches ");
				...
				// Update latest matched index from walkedHistory
				...
				//(how do you detect stairs?)
				if(navPathEdges.get(conf.npPointer).isStairs()){
					// Don't use step length because of stairs (<- what does it mean?)
...
//				Log.i("FOOTPATH", "Unmatched steps  = " + conf.npUnmatchedSteps);
				...
				// Do not look ahead if the direction matches the last edge.
